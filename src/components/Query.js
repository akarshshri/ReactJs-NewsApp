...
const Query = (props)=> {
    ...
    /*     async updateNews(){
        ...
        useEffect(async () => {
            //effect
            setTimeout(async () =>  {
                let url = `https://newsapi.org/v2/everything?q=${props.query}&apiKey=e50eef0d172c4470bed2a3e70d2475ad`;
        ...
        return (
            <>
                ...
                <div className="container pt-4" style={{marginTop : '4rem'}}>
                    <h2 className={`text-center text-${props.mode === 'light' ? 'dark' : 'light'}`}>Daily Highlighted News - {props.value}</h2>
                    {loading && <Spinner />}
                    <InfiniteScroll
